...
익스텐션을 이용해 존재하는 타입에 인스턴스 매소드나 타입 매소드를 추가할 수 있습니다. 다음 예제는 Int 타입에 repetitions라는 인스턴스 매소드를 추가한 예제 입니다.
...
repetitions\(task:\) 매소드는 \(\) -&gt; Void 타입의 하나의 인자를 받고 파라미터와 반환 값이 없는 함수입니다.
...
### 변경 가능한 인스턴스 매소드 \(Mutating Instance Methods\)

익스텐션에서 추가된 인스턴스 매소드는 인스턴스 자신\(self\)을 변경할 수 있습니다. 구조체와 열거형 매소드 중 자기 자신\(self\)를 변경하는 인스턴스 매소드는 원본 구현의 mutating 매소드와 같이 반드시 mutating으로 선언돼야 합니다.

아래 코드는 새 mutating 매소드를 추가하고 호출하는 예제입니다.
